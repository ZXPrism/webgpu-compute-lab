...
fn compute(
    ...
    // optimize: by using double-buffering, we can removed one workgroup barrier, compared to reduce_basic.wgsl
    var front = 0u;
    var back = 1u;
    ...
    for (var current_size = SEGMENT_LENGTH >> 1; current_size >= 1; current_size >>= 1) {
        ...
        if local_id.x < current_size {
            ...
            workgroup_data[back_base_addr + local_id.x] =
                workgroup_data[front_base_addr + local_id.x * 2] + workgroup_data[front_base_addr + local_id.x * 2 + 1];
        }
    ...
    if local_id.x == 0 {
        ...
        output_sum_per_segment[segment_id.x] = workgroup_data[base_addr];
    }
}
