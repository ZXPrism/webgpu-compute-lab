...
var<workgroup> psum_final : array<u32, SEGMENT_LENGTH>;
var<workgroup> slot_size : array<u32, 1u << RADIX_BITS>;
...
fn compute(
    ...
    for (var slot = 0u; slot < psum_way_cnt; slot++) {
        ...
        if local_id.x == SEGMENT_LENGTH - 1 {
            slot_size[slot] = psum[SEGMENT_LENGTH - 1];
        }
    ...
    psum_final[local_id.x] = psum_curr;
    ...
    // 2. prefix sum on slot sizes
    let SLOT_SIZE_LENGTH = 1u << RADIX_BITS;
    // pass 1: upsweep
    {
        var d = 1u;
        var offset = 1u << d;
        while offset <= SLOT_SIZE_LENGTH {
            let idx = ((local_id.x + 1u) << d) - 1u;

            if idx < SLOT_SIZE_LENGTH {
                slot_size[idx] += slot_size[idx - (offset >> 1u)];
            }
    ...
    if local_id.x == SLOT_SIZE_LENGTH - 1u {
        slot_size[SLOT_SIZE_LENGTH - 1u] = 0u;
    }
    ...
    // pass 2: downsweep
    {
        var d = SLOT_SIZE_LENGTH;
        while d >= 2u {
            let new_d = d >> 1u;
            let curr_idx = ((local_id.x + 1u) * d) - 1u;
            if curr_idx < SLOT_SIZE_LENGTH {
                let t = slot_size[curr_idx - new_d];
                slot_size[curr_idx - new_d] = slot_size[curr_idx];
                slot_size[curr_idx] += t;
            }
    ...
    // 3. scatter
    if global_id.x < array_length {
        let base_addr = SEGMENT_LENGTH * segment_id.x;
        sorted_array[base_addr + slot_size[curr_slot] + psum_final[local_id.x]] = input_data[local_id.x];
    }
}
